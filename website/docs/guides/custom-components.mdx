...
### Selecting a Week by Clicking the Week Number

- Use a custom [WeekNumber](../api/functions/WeekNumber.md) component to handle the click event and set the week as selected range.
- Call the [useDayPicker](../api/functions/useDayPicker.md) hook to access the `setSelected` function from the DayPicker context.
- Keep the components accessible by adding `role="button"` and `tabIndex={0}` to the week number header, allowing keyboard navigation.

```tsx title="./SelectWeek.tsx"
import React from "react";
...
function WeekNumber({ week, ...thProps }: WeekNumberProps) {
  ...
  const selectWeek = () => {
    ...
    if (start && end) setSelected({ from: start, to: end });
  };
