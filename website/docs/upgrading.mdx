...
DayPicker v10 does not change behavior for non-deprecated APIs in most cases. The main exception is `navLayout`: when omitted, it now defaults to `"after"`.
:::
...
| `fromDate` | [`hidden`](./api/interfaces/PropsBase#hidden) with `before` matcher and optionally [`startMonth`](./api/interfaces/PropsBase#startmonth) |
| `toDate` | [`hidden`](./api/interfaces/PropsBase#hidden) with `after` matcher and optionally [`endMonth`](./api/interfaces/PropsBase#endmonth) |
...
| `onDayKeyUp`<br/>`onDayKeyPress`<br/>`onDayPointerEnter`<br/>`onDayPointerLeave`<br/>`onDayTouchCancel`<br/>`onDayTouchEnd`<br/>`onDayTouchMove`<br/>`onDayTouchStart` | Custom [`DayButton`](./api/functions/DayButton) component |
...
- visual placement of navigation buttons
- focus order and keyboard navigation sequence
- tests/snapshots that assert DOM order or `data-nav-layout` attributes
