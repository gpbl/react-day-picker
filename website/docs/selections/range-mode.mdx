...
| `rangeResetOnSelect`  | `boolean`                                                                           | Reset range when a date is selected.       |
...
By default, the `DayPicker` keeps the range complete, changing the `from` or `to` when the user clicks on a day. To reset the range when clicking on a day, use the `rangeResetOnSelect` prop.
