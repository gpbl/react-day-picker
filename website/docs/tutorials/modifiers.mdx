...
In the [Selecting Days tutorial](selecting-days), we see how a `selected`
modifier prop changes the appearance of a selected day. This property is
available from the `onDayClick` callback.
...
A disabled day is a day that a user cannot interact with. Use the `disabled`
modifier prop to define which day should be disabled.

```jsx {7-13} showOutput open=no
function App() {
  const handleDayClick = (day) => window.alert(day.toLocaleDateString());
  return (
    <DayPicker
      numberOfMonths={2}
      initialMonth={new Date(2022, 4)}
...
```jsx showOutput open=no
function Book() {
  const bookedDays = [new Date(2021, 5, 8), new Date(2021, 5, 9), new Date(2021, 5, 10)];
  const bookedStyle = { color: 'tomato' };
