...
- To use the Latin numeral system, pass the `numerals` prop:
...
DayPicker supports the [Hebrew calendar](https://en.wikipedia.org/wiki/Hebrew_calendar), a lunisolar calendar used for Jewish religious observances and holidays.

- To use the Hebrew calendar, import `DayPicker` from `react-day-picker/hebrew`:
...
DayPicker supports the [Islamic Umm al-Qura calendar](https://en.wikipedia.org/wiki/Umm_al-Qura_calendar), the civil calendar used in Saudi Arabia and many other regions.

- To use the Hijri calendar, import `DayPicker` from `react-day-picker/hijri`:
...
By default, the Hijri calendar uses the Arabic (Saudi Arabia) locale, renders right-to-left, and formats numbers with Arabic-Indic digits. Switch to English labels with `enUS` and Latin numerals:
