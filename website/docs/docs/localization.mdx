...
By default, the Ethiopic calendar uses the geez numeral system.

- To use the Latn (English) locale, import it and pass the `numerals` prop.

```tsx
import { DayPicker, enUS } from "react-day-picker/ethiopic";

export function EthiopicLatn() {
  return <DayPicker numerals="latn" />;
}
```

<BrowserWindow>
  <Examples.EthiopicLatn />
</BrowserWindow>

### Formatting Ethiopic Dates

The calendar always returns Gregorian dates. Use the exported `getDateLib` getter to format dates to Ethiopic calendar.
