...
When working with time zones, use the `TZDate` object exported by `react-day-picker` instead of the native `Date` object. This ensures proper handling of time zones. For more information, refer to the [date-fns documentation](https://date-fns.org/v4.1.0/docs/Time-Zones).
