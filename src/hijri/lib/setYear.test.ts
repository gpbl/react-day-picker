...
describe("setYear", () => {
  describe("when the source day does not exist in the target Hijri year", () => {
    test("clamps day to the last day of the target month", () => {
      ...
      const result = setYear(start, found.year + 1);
  ...
  describe("when setting year past the maximum supported range", () => {
    test("returns the maximum supported date", () => {
      const result = setYear(new Date(GREGORIAN_MAX_DATE), 1501);

      expect(result).toEqual(new Date(GREGORIAN_MAX_DATE));
    });
  });

  describe("when setting year before the minimum supported range", () => {
    test("returns the minimum supported date", () => {
      const result = setYear(new Date(GREGORIAN_MIN_DATE), 1342);
