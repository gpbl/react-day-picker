...
describe("setMonth", () => {
  describe("when the source day does not exist in the target Hijri month", () => {
    test("clamps day to the last day of the target month", () => {
      ...
      const result = setMonth(start, found.monthIndex + 1);
  ...
  describe("when setting month past the maximum supported range", () => {
    test("returns the maximum supported date", () => {
      const result = setMonth(new Date(GREGORIAN_MAX_DATE), 12);

      expect(result).toEqual(new Date(GREGORIAN_MAX_DATE));
    });
  });

  describe("when setting month before the minimum supported range", () => {
    test("returns the minimum supported date", () => {
      const result = setMonth(new Date(GREGORIAN_MIN_DATE), -1);
