...
function renderHook(
  ...
  const gridcellRef = createRef<HTMLDivElement>();
  return renderDayPickerHook(
    () => useDayRender(date, displayMonth),
    dayPickerProps,
    contexts
  );
}
...
describe('when rendering the today’s date', () => {
  ...
  test('the div should include the default class name', () => {
    ...
    expect(result.current.htmlAttributes.className?.split(' ')).toContain(
      defaultClassNames.day
    );
  });
  test('the gridcell should include the default class name', () => {
    const result = renderHook(date, displayMonth);
    expect(result.current.htmlAttributes.className?.split(' ')).toContain(
      defaultClassNames.day
    );
  });
  test('the gridcell should not have "aria-selected"', () => {
    const result = renderHook(date, displayMonth);
    expect(result.current.htmlAttributes['aria-selected']).toBeUndefined();
  });
  test('the gridcell should have 0 as "tabIndex"', () => {
    const result = renderHook(date, displayMonth);
    expect(result.current.htmlAttributes.tabIndex).toBe(0);
  });
  ...
  test.each(testEvents)(
    'the gridcell should have the "%s" event handler',
    (eventName) => {
      const result = renderHook(date, displayMonth);
      expect(result.current.htmlAttributes[eventName]).toBeDefined();
    }
  );
...
describe('when "modifiersStyles" is passed in', () => {
  ...
  test('the div props should include the modifiers style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.htmlAttributes.style).toStrictEqual(
      dayPickerProps.modifiersStyles.foo
    );
  });
  test('the gridcell props should include the modifiers style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.htmlAttributes.style).toStrictEqual(
      dayPickerProps.modifiersStyles.foo
    );
...
describe('when "styles.day" is passed in', () => {
  ...
  test('the gridcell props should include the style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.htmlAttributes.style).toStrictEqual(
      dayPickerProps.styles.day
    );
...
describe('when "modifiersClassNames" is passed in', () => {
  ...
  test('the div props should include the modifiers classNames', () => {
    expect(result.current.htmlAttributes.className).toContain(
      dayPickerProps.modifiersClassNames.foo
    );
  });
  test('the gridcell props should include the modifiers classNames', () => {
    expect(result.current.htmlAttributes.className).toContain(
      dayPickerProps.modifiersClassNames.foo
    );
  });
...
describe('when "classNames.day" is passed in', () => {
  ...
  test('the gridcell props should include the class name', () => {
    expect(result.current.htmlAttributes.className).toContain(
      dayPickerProps.classNames.day
    );
  });
...
describe('when the day is not target of focus', () => {
  ...
  test('the gridcell should have tabIndex -1', () => {
...
describe('when the day is target of focus', () => {
  ...
  test('the gridcell should have tabIndex 0', () => {
    expect(result.current.htmlAttributes.tabIndex).toBe(0);
  });
});
...
describe('when the day is disabled', () => {
  ...
  const result = renderHook(date, date, dayPickerProps);
  test('the gridcell should be disabled', () => {
    expect(result.current.htmlAttributes['aria-disabled']).toBe(true);
  });
});
...
describe('when the day is selected', () => {
  ...
  const result = renderHook(date, date, dayPickerProps);
  test('the gridcell should be "aria-selected"', () => {
    expect(result.current.htmlAttributes['aria-selected']).toBe(true);
  });
});
