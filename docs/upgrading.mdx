...
DayPicker follows the [Semantic Versioning](https://semver.org/) and tries to keep the API as stable as possible. However, sometimes breaking changes are necessary to improve the library. This guide will help you to upgrade from previous versions of DayPicker.
...
3. **Update the CSS import**: the stylesheet has been moved to `/dist`. For example:
   ...
   ```
4. **Update your custom styles**. See [Styling DayPicker](https://daypicker.dev/basics/styling) for more information.
...
- Try with [useInput](https://daypicker.dev/guides/input-fields) hook.
- See [this example](https://daypicker.dev/guides/input-fields#example-date-picker-dialog) to build a date picker with v8 in a dialog.
