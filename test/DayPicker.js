...
describe('DayPicker', () => {
  it('should work with commonjs require', () => {
    const DP = require('../DayPicker');
    expect(DP).to.be.a('function');
  });
  ...
  it('renders the months on a reverse order as specified by reverseMonths', () => {
    const dayPickerEl = TestUtils.renderIntoDocument(
      <DayPicker initialMonth={new Date(2015, 11, 6)} numberOfMonths={4} reverseMonths />
    );
  ...
  it('calls event handlers on a day cell', () => {
    ...
    const handlerCallback = [
      ...
      sinon.match(mods => mods instanceof Array && mods[0] === 'firstDay', 'modifiers'),
    ];
