...
function readPackageSidebar(slug) {
  ...
  const sidebarPath = path.join(apiDir, slug, "typedoc-sidebar.cjs");
...
function verifyPackageSidebars() {
  for (const { slug } of packages) {
    readPackageSidebar(slug);
  }
}

fs.mkdirSync(apiDir, { recursive: true });
for (const outputName of rootApiOutputNames) {
  fs.rmSync(path.join(apiDir, outputName), {
    recursive: true,
    force: true,
  });
}
for (const pkg of packages) {
  fs.rmSync(path.join(apiDir, pkg.slug), { recursive: true, force: true });
}

process.chdir(repoRoot);

for (const pkg of packages) {
  const outDir = path.join(apiDir, pkg.slug);
