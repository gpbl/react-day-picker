...
To use the included styles, add `@daypicker/react/style.css` to your HTML document. This will apply the `.rdp` class names used by DayPicker.
...
If you are not using a bundler, you can copy the CSS file to your project. See the [style.css](https://github.com/gpbl/react-day-picker/blob/release/v10-next/src/style.css) file in the DayPicker repository for the source.
...
| ------------ | ------------------------------------------------- | ------------------------------------------------- |
| `classNames` | [`ClassNames`](../api/react/type-aliases/ClassNames.md) | Apply custom class names instead of the defaults. |
...
Use the `classNames` prop to apply custom class names instead of the default ones. The [`ClassNames`](../api/react/type-aliases/ClassNames.md) type lists all the class names used by DayPicker.
...
- Extend the default class names with [`getDefaultClassNames`](../api/react/functions/getDefaultClassNames.md).
- Read the [`style.css`](https://github.com/gpbl/react-day-picker/blob/release/v10-next/src/style.css) file from the source and get familiar with the [UI elements](../docs/anatomy.mdx).
...
<BrowserWindow sourceUrl="https://github.com/gpbl/react-day-picker/blob/release/v10-next/examples/TailwindCSS.tsx">
